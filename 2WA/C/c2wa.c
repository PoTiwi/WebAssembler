...
int main(int argc, char *argv[]) {
    ...
    if(argc<3){
        fprintf(stderr,"c2iwa — Full C to IWA transpiler (Comprehensive Edition)\n");
        fprintf(stderr,"Usage: %s <input.c> <output.iwa>\n",argv[0]);
        fprintf(stderr,"\nThe output .iwa file can then be assembled with iwa_assembler\n");
        fprintf(stderr,"into .wassm bytecode for the WebAssembler VM.\n");
        return 1;
    }
    ...
    /* File header */
    emit("; IWA generated by c2iwa (Comprehensive Edition)");
    emit("; Source: %s",argv[1]);
    emit("; Assemble: iwa_assembler %s output.wassm",argv[2]);
    emit(";");
    emit("; Standard library builtins: strlen strcmp strcpy strcat memcpy memset");
    emit(";                            atoi itoa rand geti gets puts inc dec abs");
    emit(";");
    emit(";");
    ...
    fprintf(stdout,"transpiled %d IWA lines → %s\n",out_n,argv[2]);
    out_buf_free();
    return 0;
}
