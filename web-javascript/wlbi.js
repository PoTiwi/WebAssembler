...
(function (root, factory) {
    ...
    while (i < s.length) {
      if (s[i] === '\\' && i + 1 < s.length) {
        ...
        switch (s[i]) {
          ...
          case 'r':  out += '\r';      break;
          case '0':  out += '\0';      break;
          case 'a':  out += '\x07';    break;  // BEL — FIX #9
          case 'b':  out += '\x08';    break;  // BS  — FIX #9
          case '\\': out += '\\';      break;
  ...
  /**
   * FIX #1 & #2: carry uses unsigned inputs; overflow uses independent
   * sign tests for a and b (not b+cin).
   */
  function addWithFlags(regs, a, b, cin = 0n) {
    // Unsigned arithmetic for carry
    const ua = BigInt.asUintN(64, a);
    const ub = BigInt.asUintN(64, b);
    const ur = ua + ub + cin;

    const u64 = BigInt.asUintN(64, ur);
    const i64 = BigInt.asIntN(64, u64);

    regs.N = i64 < 0n;
    regs.Z = u64 === 0n;
    regs.C = ur >= (1n << 64n);                // FIX #1: pure unsigned carry

    // Overflow: same-sign inputs produce different-sign output
    const sa = BigInt.asIntN(64, a) < 0n;
    const sb = BigInt.asIntN(64, b) < 0n;      // FIX #2: b, not b+cin
    const sr = i64 < 0n;
    regs.V = (sa === sb) && (sa !== sr);
  ...
  class Registers {
    constructor() {
      ...
      this.lr  = 0n;                      // link register (x30)
      this.N   = false;
      this.Z   = false;
      this.C   = false;
      this.V   = false;
      this.stack   = [];                  // PUSH/POP operand stack
      this.mem     = new Map();           // word-addressed memory
      this.strings = new Map();           // per-register string values
    }

    /* Register families:
     *  0 → x<n>   (64-bit, signed store)
     *  1 → w<n>   (32-bit, zero-extended)
     *  2 → sp
     *  3 → lr / x30
     *  4 → xzr   (always zero, writes discarded)
     *  7 → fp / x29
     *  8 → x16 (ip0)
     *  9 → x17 (ip1)
     */
    readInt(enc) {
      ...
      const fam = colon < 0 ? 0 : Number(enc.slice(0, colon));
      ...
      switch (fam) {
        ...
        case 4: return 0n;   // xzr
        case 5: return 0n;
        case 6: return 0n;
        case 7: return this.x[29] ?? 0n;  // fp
        case 8: return this.x[16] ?? 0n;  // ip0
        case 9: return this.x[17] ?? 0n;  // ip1
        default: return 0n;
      }
    }
    ...
    readFloat(enc) {
      ...
      const num = colon < 0 ? 0  : Number(enc.slice(colon + 1));
      if (fam === 10) return this.d[num] ?? 0.0;           // d (double)
      if (fam === 11) return Math.fround(this.d[num] ?? 0.0); // s (single)
      return 0.0;
    }
    ...
    isFloat(enc) {
      const fam = Number(enc.slice(0, enc.indexOf(':')));
  ...
  /* =========================================================
   * Parse .wassm source → array of token arrays   FIX #8
   * ========================================================= */
  function parseWassm(src) {
    const code = [];
    for (const raw of src.split('\n')) {
      const line = raw.trim();
      if (!line || line.startsWith(';')) continue;  // FIX #8: skip blanks/comments
  ...
  class VM {
    ...
    /**
     * Run the program.  Yields to the event loop every `yieldEvery` steps
     * to keep the browser responsive.
     */
    async run(yieldEvery = 50_000) {
      yieldEvery = Math.max(1, Math.min(1_000_000, yieldEvery | 0));  // FIX #10
      let steps = 0;
      while (this.running && this.pc >= 0 && this.pc < this.code.length) {
        const instr = this.code[this.pc];
        if (!instr || instr.length === 0) { this.pc++; continue; }
        await this.step(instr);
        if (++steps % yieldEvery === 0) await new Promise(r => setTimeout(r, 0));
      }
    }
    ...
    async step(tok) {
      ...
      switch (op) {
        ...
        case OP.LDS: {
          ...
          regs.writeStr(dst, str);
          regs.writeInt(dst, BigInt(str.length));
          this.pc++; break;
        }

        /* ── Address-of-label / string-length ────────────────── */
        // FIX #3: case OP.STRLEN (226) folded here; removed duplicate below
        case OP.ADL:
        case OP.STRLEN: {
          const src = tok[2] || '';  // FIX #11: default to '' when absent
          const s   = regs.readStr(src);
        ...
        case OP.STRH: {
          ...
          regs.memWriteByte(addr, v & 0xffn);
          regs.memWriteByte(addr + 1n, (v >> 8n) & 0xffn);
          this.pc++; break;
        }
        ...
        case OP.TST: {
          const b = tok[2] === 'R' ? regs.readInt(tok[3]) : BigInt(tok[3]);
          setFlags(regs, regs.readInt(tok[1]) & b);
          this.pc++; break;
        }
        ...
        case OP.BLR:
          ...
          break;
        case OP.RET: {
          // FIX #4: use tok.length > 1 to test arg presence, not truthiness
        ...
        /* ── SVC (system call) ────────────────────────────────── */
        case OP.SVC: {
          const no = Number(regs.readInt('0:8'));  // x8 = syscall number
          switch (no) {
            case 1:    // write (Linux AArch64)
            case 64: { // also write
              // FIX #7: x0 = fd, x1 = buf; emit the string stored in x1
        ...
        /* FIX #14: CCMP/CCMN condition token is tok[4], not tok[5] */
        case OP.CCMP: {
          if (evalCond(tok[4], regs)) {
            const b = tok[2] === 'R' ? regs.readInt(tok[3]) : BigInt(tok[3]);
            subWithFlags(regs, regs.readInt(tok[1]), b);
          } else {
            const n = Number(tok[4]);
            regs.N = !!(n & 8); regs.Z = !!(n & 4); regs.C = !!(n & 2); regs.V = !!(n & 1);
          }
          this.pc++; break;
        }
        ...
        case OP.STLRH: case OP.STXRH: case OP.STLXRH: {
          ...
          regs.memWriteByte(base, v & 0xffn);
          regs.memWriteByte(base + 1n, (v >> 8n) & 0xffn);
        ...
        case OP.POP: {
          ...
          if (item !== undefined) {
            ...
            if (item.str !== null) regs.writeStr(tok[1], item.str);
          }
          this.pc++; break;
        }
        ...
        case OP.SWP: {
          ...
          regs.writeInt(tok[1], b); regs.writeInt(tok[2], a);
          regs.writeStr(tok[1], sb); regs.writeStr(tok[2], sa);
          this.pc++; break;
        }
        ...
        case OP.GETI: {
          ...
          // FIX #5: explicit NaN guard before BigInt conversion
        ...
        case OP.MAX2: {
          ...
          regs.writeInt(tok[1], a > b ? a : b); this.pc++; break;
        }
        case OP.MIN2: {
          const a = regs.readInt(tok[2]), b = regs.readInt(tok[3]);
          regs.writeInt(tok[1], a < b ? a : b); this.pc++; break;
        ...
        case OP.MOD: {
          // FIX #6: consistent with C truncated-division semantics
          const a = regs.readInt(tok[2]), b = regs.readInt(tok[3]);
          regs.writeInt(tok[1], b === 0n ? 0n : a % b);
          this.pc++; break;
        }
        ...
        case OP.MEMCPY: {
          ...
          if (s !== null) {
            regs.writeStr(tok[1], s.slice(0, len));
          } else {
            // FIX #12: use BigInt for byte loop to avoid mixed-type errors
            const bs = regs.readInt(tok[2]), bd = regs.readInt(tok[1]);
            for (let i = 0n; i < BigInt(len); i++) regs.memWriteByte(bd + i, regs.memReadByte(bs + i));
          }
          this.pc++; break;
        }
        case OP.MEMSET: {
        ...
        case OP.ATOI: {
          // FIX #13: strip trailing non-digits to mirror C atoi
          const raw = regs.readStr(tok[2]) ?? '0';
  ...
  const api = {
    ...
    /** Called with text written to stderr / debug output. */
     ...
     * The default uses browser prompt(); replace this with your own UI hook.
     */
    ...
    /**
     * Load bytecode from a URL via fetch.
     * @param {string}   url       — path / URL to the .wassm file
     * @param {Function} [callback] — called (no args) when ready
    ...
    /**
     * Load bytecode from a raw string (no network request needed).
    ...
    /**
     * Load bytecode from a File or Blob.
