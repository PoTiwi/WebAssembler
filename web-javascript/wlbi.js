/**
 * WebAssembler Virtual Machine — Browser Edition
 * Executes .wassm bytecode produced by the WebAssembler compiler (main.c)
 *
 * Usage:
 *   webassembler.init("file.wassm", () => {
 *     webassembler.execute();
 *   });
 *
 *   // Or with inline bytecode string:
 *   webassembler.initFromString(bytecodeSrc, () => {
 *     webassembler.execute();
 *   });
 *
 * I/O hooks (set before calling execute):
 *   webassembler.onOutput = (text) => { ... }   // stdout
 *   webassembler.onError  = (text) => { ... }   // stderr
 *   webassembler.onInput  = () => Promise<string>  // stdin (async, returns Promise<string>)
 *
 * webassembler.execute() returns a Promise that resolves when the program halts.
 */
...
(function (root, factory) {
  if (typeof module !== 'undefined' && module.exports) {
    ...
    root.webassembler = factory();            // Browser global
  }
}(typeof globalThis !== 'undefined' ? globalThis : this, function () {

  /* =========================================================
   * Opcode constants  (mirror of main.c enum)
   * ========================================================= */
  const OP = {
  ...
  function setFlags(regs, result) {
    const i64 = BigInt.asIntN(64, result);
    regs.N = i64 < 0n;
    regs.Z = i64 === 0n;
  }

  function addWithFlags(regs, a, b, cin = 0n) {
    const r   = a + b + cin;
    const i64 = BigInt.asIntN(64, r);
    const u64 = BigInt.asUintN(64, r);
    regs.N = i64 < 0n;
    regs.Z = u64 === 0n;
    regs.C = r >= (1n << 64n) || r < 0n;
    const sa = BigInt.asIntN(64, a) < 0n;
    const sb = BigInt.asIntN(64, b + cin) < 0n;
    const sr = i64 < 0n;
    regs.V = (sa === sb) && (sa !== sr);
    return u64;
  }

  function subWithFlags(regs, a, b) {
    return addWithFlags(regs, a, ~b & 0xFFFFFFFFFFFFFFFFn, 1n);
  }
  ...
  function applyShift(val, type, amt) {
    ...
    switch ((type || '').toLowerCase()) {
      ...
      case 'ror': { const s = amt % 64n; return (v >> s) | (v << (64n - s)); }
      default: return val;
    }
  }

  function clz64(v) {
    let n = 0n; v = BigInt.asUintN(64, v);
    if (v === 0n) return 64n;
    for (let bit = 63n; bit >= 0n; bit--) { if ((v >> bit) & 1n) break; n++; }
    return n;
  }
  function cls64(v) {
    v = BigInt.asIntN(64, v);
    const sign = v < 0n ? 1n : 0n; let n = 0n;
    for (let bit = 62n; bit >= 0n; bit--) { if (((v >> bit) & 1n) === sign) n++; else break; }
    return n;
  }
  function rbit64(v) {
    v = BigInt.asUintN(64, v); let r = 0n;
  ...
  class Registers {
    ...
    readInt(enc) {
      const [fam, num] = enc.split(':').map(Number);
      switch (fam) {
    ...
    isFloat(enc) {
      const fam = Number(enc.split(':')[0]);
      return fam === 10 || fam === 11 || fam === 12 || fam === 13;
    }

    readStr(enc) { return this.strings.get(enc) ?? null; }
    writeStr(enc, s) { this.strings.set(enc, s); }

    memRead(addr) { return this.mem.get(addr.toString()) ?? 0n; }
    memWrite(addr, val) { this.mem.set(addr.toString(), BigInt.asIntN(64, val)); }
    memReadByte(addr) {
      const base = (addr / 8n) * 8n, off = Number(addr % 8n);
      return (this.memRead(base) >> BigInt(off * 8)) & 0xffn;
    }
    memWriteByte(addr, val) {
      const base = (addr / 8n) * 8n, off = Number(addr % 8n);
      let word = this.memRead(base);
      const mask = ~(0xffn << BigInt(off * 8));
      this.memWrite(base, (word & mask) | ((val & 0xffn) << BigInt(off * 8)));
    }
  }

  /* =========================================================
   * Parse .wassm source text → array of token arrays
   * ========================================================= */
   ...
   * ========================================================= */
  class VM {
    constructor(code, api) {
      this.code      = code;
      this.pc        = 0;
      this.regs      = new Registers();
      this.running   = true;
      this.callStack = [];
      this.api       = api;   // { onOutput, onError, onInput }
    }
    ...
    /* Run the entire program asynchronously.
       Yields to the event loop every YIELD_EVERY steps to avoid UI freeze. */
    async run(YIELD_EVERY = 50000) {
      let steps = 0;
    ...
    async execute(tok) {
      ...
      switch (op) {
        ...
        case OP.LDS: {
          const dst = tok[1], str = unescapeStr(tok[2] || '');
          regs.writeStr(dst, str); regs.writeInt(dst, BigInt(str.length));
          this.pc++; break;
        }
        ...
        case OP.MOV: {
          const dst = tok[1];
          if (tok[2] === 'R') {
            regs.writeInt(dst, regs.readInt(tok[3]));
            regs.writeStr(dst, regs.readStr(tok[3]));
          } else {
            regs.writeInt(dst, BigInt(tok[3]));
            regs.writeStr(dst, null);
          }
          this.pc++; break;
        }
        ...
        case OP.MOVK: {
          ...
          regs.writeInt(tok[1], (cur & ~(0xFFFFn << shift)) | ((imm & 0xFFFFn) << shift));
          this.pc++; break;
        }
        case OP.MVN:
          regs.writeInt(tok[1], ~regs.readInt(tok[2])); this.pc++; break;
        ...
        case OP.LDR: {
          ...
          if (tok[2] === 'R') {
            ...
            regs.writeInt(dst, regs.memRead(regs.readInt(tok[3]) + BigInt(tok[4] || 0)));
          }
          this.pc++; break;
        }
        ...
        case OP.LDRH: {
          ...
          regs.writeInt(tok[1], regs.memReadByte(addr) | (regs.memReadByte(addr + 1n) << 8n));
          this.pc++; break;
        }
        case OP.LDRSB: {
          regs.writeInt(tok[1], BigInt.asIntN(8, regs.memReadByte(regs.readInt(tok[2]) + BigInt(tok[3] || 0))));
          this.pc++; break;
        }
        case OP.LDRSH: {
          const addr = regs.readInt(tok[2]) + BigInt(tok[3] || 0);
          regs.writeInt(tok[1], BigInt.asIntN(16, regs.memReadByte(addr) | (regs.memReadByte(addr + 1n) << 8n)));
          this.pc++; break;
        }
        case OP.LDRSW: {
          const addr = regs.readInt(tok[2]) + BigInt(tok[3] || 0);
        ...
        case OP.STRB: {
          regs.memWriteByte(regs.readInt(tok[2]) + BigInt(tok[3] || 0), regs.readInt(tok[1]) & 0xffn);
          this.pc++; break;
        }
        case OP.STRH: {
          const addr = regs.readInt(tok[2]) + BigInt(tok[3] || 0);
          const v = regs.readInt(tok[1]);
          regs.memWriteByte(addr, v & 0xffn); regs.memWriteByte(addr + 1n, (v >> 8n) & 0xffn);
          this.pc++; break;
        }
        ...
        case OP.ADD:
        case OP.ADDS: {
          let a = regs.readInt(tok[2]), b;
          if (tok[3] === 'R') { b = applyShift(regs.readInt(tok[4]), tok[5], BigInt(tok[6] || 0)); }
          else { b = BigInt(tok[4]); }
          const res = op === OP.ADDS ? addWithFlags(regs, a, b) : a + b;
          regs.writeInt(tok[1], res); this.pc++; break;
        }
        ...
        case OP.SUBS: {
          ...
          const res = op === OP.SUBS ? subWithFlags(regs, a, b) : a - b;
          regs.writeInt(tok[1], res); this.pc++; break;
        }

        case OP.MUL:
          regs.writeInt(tok[1], regs.readInt(tok[2]) * regs.readInt(tok[3])); this.pc++; break;
        case OP.UDIV: {
          const a = BigInt.asUintN(64, regs.readInt(tok[2])), b = BigInt.asUintN(64, regs.readInt(tok[3]));
          regs.writeInt(tok[1], b === 0n ? 0n : a / b); this.pc++; break;
        }
        case OP.SDIV: {
          const a = BigInt.asIntN(64, regs.readInt(tok[2])), b = BigInt.asIntN(64, regs.readInt(tok[3]));
          regs.writeInt(tok[1], b === 0n ? 0n : a / b); this.pc++; break;
        }

        case OP.AND:
        case OP.ANDS: {
          const a = regs.readInt(tok[2]), b = tok[3] === 'R' ? regs.readInt(tok[4]) : BigInt(tok[4]);
          const res = a & b; regs.writeInt(tok[1], res);
          if (op === OP.ANDS) setFlags(regs, res);
          this.pc++; break;
        }
        case OP.ORR: {
          const b = tok[3] === 'R' ? regs.readInt(tok[4]) : BigInt(tok[4]);
          regs.writeInt(tok[1], regs.readInt(tok[2]) | b); this.pc++; break;
        }
        case OP.EOR: {
          const b = tok[3] === 'R' ? regs.readInt(tok[4]) : BigInt(tok[4]);
          regs.writeInt(tok[1], regs.readInt(tok[2]) ^ b); this.pc++; break;
        ...
        case OP.BICS: {
          ...
          regs.writeInt(tok[1], res); setFlags(regs, res); this.pc++; break;
        }
        ...
        case OP.ROR: {
          ...
          const s = BigInt(Number(tok[3] === 'R' ? (regs.readInt(tok[4]) & 63n) : BigInt(tok[4])) % 64);
          regs.writeInt(tok[1], (a >> s) | (a << (64n - s)));
          this.pc++; break;
        }

        case OP.CMP: {
          const b = tok[2] === 'R' ? regs.readInt(tok[3]) : BigInt(tok[3]);
          subWithFlags(regs, regs.readInt(tok[1]), b); this.pc++; break;
        }
        case OP.CMN: {
          const b = tok[2] === 'R' ? regs.readInt(tok[3]) : BigInt(tok[3]);
          addWithFlags(regs, regs.readInt(tok[1]), b); this.pc++; break;
        }
        case OP.TST: {
          const b = tok[2] === 'R' ? regs.readInt(tok[3]) : BigInt(tok[3]);
          setFlags(regs, regs.readInt(tok[1]) & b); this.pc++; break;
        }

        case OP.NEG:  regs.writeInt(tok[1], -regs.readInt(tok[2])); this.pc++; break;
        case OP.NEGS: { const r = subWithFlags(regs, 0n, regs.readInt(tok[2])); regs.writeInt(tok[1], r); this.pc++; break; }

        case OP.ADC:  regs.writeInt(tok[1], regs.readInt(tok[2]) + regs.readInt(tok[3]) + (regs.C ? 1n : 0n)); this.pc++; break;
        case OP.ADCS: { const r = addWithFlags(regs, regs.readInt(tok[2]), regs.readInt(tok[3]) + (regs.C ? 1n : 0n)); regs.writeInt(tok[1], r); this.pc++; break; }
        case OP.SBC:  regs.writeInt(tok[1], regs.readInt(tok[2]) - regs.readInt(tok[3]) - (regs.C ? 0n : 1n)); this.pc++; break;
        case OP.SBCS: { const r = subWithFlags(regs, regs.readInt(tok[2]), regs.readInt(tok[3]) + (regs.C ? 0n : 1n)); regs.writeInt(tok[1], r); this.pc++; break; }

        case OP.MADD: regs.writeInt(tok[1], regs.readInt(tok[2]) * regs.readInt(tok[3]) + regs.readInt(tok[4])); this.pc++; break;
        case OP.MSUB: regs.writeInt(tok[1], regs.readInt(tok[4]) - regs.readInt(tok[2]) * regs.readInt(tok[3])); this.pc++; break;
        case OP.MNEG: regs.writeInt(tok[1], -(regs.readInt(tok[2]) * regs.readInt(tok[3]))); this.pc++; break;

        case OP.SMULL: regs.writeInt(tok[1], BigInt.asIntN(32, regs.readInt(tok[2])) * BigInt.asIntN(32, regs.readInt(tok[3]))); this.pc++; break;
        case OP.UMULL: regs.writeInt(tok[1], BigInt.asUintN(32, regs.readInt(tok[2])) * BigInt.asUintN(32, regs.readInt(tok[3]))); this.pc++; break;
        case OP.SMULH: regs.writeInt(tok[1], (BigInt.asIntN(64, regs.readInt(tok[2])) * BigInt.asIntN(64, regs.readInt(tok[3]))) >> 64n); this.pc++; break;
        case OP.UMULH: regs.writeInt(tok[1], (BigInt.asUintN(64, regs.readInt(tok[2])) * BigInt.asUintN(64, regs.readInt(tok[3]))) >> 64n); this.pc++; break;

        case OP.SMADDL: regs.writeInt(tok[1], BigInt.asIntN(32, regs.readInt(tok[2])) * BigInt.asIntN(32, regs.readInt(tok[3])) + regs.readInt(tok[4])); this.pc++; break;
        case OP.UMADDL: regs.writeInt(tok[1], BigInt.asUintN(32, regs.readInt(tok[2])) * BigInt.asUintN(32, regs.readInt(tok[3])) + regs.readInt(tok[4])); this.pc++; break;
        case OP.SMSUBL: regs.writeInt(tok[1], regs.readInt(tok[4]) - BigInt.asIntN(32, regs.readInt(tok[2])) * BigInt.asIntN(32, regs.readInt(tok[3]))); this.pc++; break;
        case OP.UMSUBL: regs.writeInt(tok[1], regs.readInt(tok[4]) - BigInt.asUintN(32, regs.readInt(tok[2])) * BigInt.asUintN(32, regs.readInt(tok[3]))); this.pc++; break;

        // --- Branches ---
        case OP.B:   this.pc = Number(tok[1]); break;
        case OP.BL:  regs.lr = BigInt(this.pc + 1); this.callStack.push(this.pc + 1); this.pc = Number(tok[1]); break;
        case OP.BR:  this.pc = Number(regs.readInt(tok[1])); break;
        case OP.BLR: regs.lr = BigInt(this.pc + 1); this.callStack.push(this.pc + 1); this.pc = Number(regs.readInt(tok[1])); break;
        case OP.RET: {
          const ret = tok[1] ? regs.readInt(tok[1]) : regs.lr;
          if (this.callStack.length > 0) { this.pc = this.callStack.pop(); }
          else { this.pc = Number(ret); if (this.pc < 0 || this.pc >= this.code.length) this.running = false; }
          break;
        }
        ...
        case OP.TBZ:  { const b = BigInt(tok[2]); this.pc = ((regs.readInt(tok[1]) >> b) & 1n) === 0n ? Number(tok[3]) : this.pc + 1; break; }
        case OP.TBNZ: { const b = BigInt(tok[2]); this.pc = ((regs.readInt(tok[1]) >> b) & 1n) !== 0n ? Number(tok[3]) : this.pc + 1; break; }

        case OP.ADR:
        case OP.ADRP:
          regs.writeInt(tok[1], BigInt(Number(tok[2]))); this.pc++; break;
        ...
        case OP.SVC: {
          ...
          switch (no) {
            ...
            case 63: {
              ...
              regs.writeStr('0:0', line); regs.writeInt('0:0', BigInt(line.length));
              break;
            }
            case 60:
            case 93: this.running = false; break;
            default: break;
          }
          this.pc++; break;
        }
        ...
        case OP.HLT:
        case OP.BRK:
          this.running = false; this.pc++; break;
        ...
        case OP.MSR: case OP.SYS: case OP.SYSL: case OP.IC: case OP.DC: case OP.AT: case OP.TLBI:
          this.pc++; break;
        ...
        case OP.EXTR: {
          const n = BigInt.asUintN(64, regs.readInt(tok[2])), m = BigInt.asUintN(64, regs.readInt(tok[3]));
        ...
        case OP.SBFM: {
          ...
          const width = immS + 1n; let field = (src >> immR) & ((1n << width) - 1n);
          if ((field >> (width - 1n)) & 1n) field |= ~((1n << width) - 1n);
          regs.writeInt(tok[1], field); this.pc++; break;
        }
        case OP.UBFM: {
          const src = regs.readInt(tok[2]), immR = BigInt(tok[3]), immS = BigInt(tok[4]);
          regs.writeInt(tok[1], (src >> immR) & ((1n << (immS + 1n)) - 1n)); this.pc++; break;
        }
        case OP.BFM: {
          const src = regs.readInt(tok[2]), immR = BigInt(tok[3]), immS = BigInt(tok[4]);
          const width = immS + 1n, field = (src >> immR) & ((1n << width) - 1n);
          const mask = ((1n << width) - 1n) << immR;
          regs.writeInt(tok[1], (regs.readInt(tok[1]) & ~mask) | (field << immR)); this.pc++; break;
        }

        case OP.SBFX: {
          const s = regs.readInt(tok[2]), lsb = BigInt(tok[3]), w = BigInt(tok[4]);
          let f = (s >> lsb) & ((1n << w) - 1n); if ((f >> (w-1n)) & 1n) f |= ~((1n<<w)-1n);
          regs.writeInt(tok[1], f); this.pc++; break;
        }
        case OP.UBFX: {
          const s = regs.readInt(tok[2]), lsb = BigInt(tok[3]), w = BigInt(tok[4]);
          regs.writeInt(tok[1], (s >> lsb) & ((1n << w) - 1n)); this.pc++; break;
        }
        case OP.SBFIZ: {
          const s = regs.readInt(tok[2]), lsb = BigInt(tok[3]), w = BigInt(tok[4]);
          let f = s & ((1n << w) - 1n); if ((f >> (w-1n)) & 1n) f |= ~((1n<<w)-1n);
          regs.writeInt(tok[1], f << lsb); this.pc++; break;
        }
        case OP.UBFIZ: {
          const s = regs.readInt(tok[2]), lsb = BigInt(tok[3]), w = BigInt(tok[4]);
          regs.writeInt(tok[1], (s & ((1n << w) - 1n)) << lsb); this.pc++; break;
        }
        case OP.BFXIL: {
          const s = regs.readInt(tok[2]), lsb = BigInt(tok[3]), w = BigInt(tok[4]);
          const f = (s >> lsb) & ((1n << w) - 1n), mask = (1n << w) - 1n;
          regs.writeInt(tok[1], (regs.readInt(tok[1]) & ~mask) | f); this.pc++; break;
        }
        case OP.BFI: {
          const s = regs.readInt(tok[2]), lsb = BigInt(tok[3]), w = BigInt(tok[4]);
          const f = s & ((1n << w) - 1n), mask = ((1n << w) - 1n) << lsb;
          regs.writeInt(tok[1], (regs.readInt(tok[1]) & ~mask) | (f << lsb)); this.pc++; break;
        }
        ...
        case OP.FMOV: {
          ...
          } else { regs.writeFloat(tok[1], parseFloat(tok[3])); }
          this.pc++; break;
        }
        ...
        case OP.FCMP:
        case OP.FCMPE: {
          const a = regs.readFloat(tok[1]), b = tok[2] === 'R' ? regs.readFloat(tok[3]) : 0.0;
          regs.Z = a===b; regs.N = a<b; regs.C = a>=b; regs.V = isNaN(a)||isNaN(b);
          this.pc++; break;
        }
        case OP.FCCMP:
        case OP.FCCMPE: {
        ...
        case OP.FCSEL: {
          regs.writeFloat(tok[1], evalCond(tok[4], regs) ? regs.readFloat(tok[2]) : regs.readFloat(tok[3]));
          this.pc++; break;
        }
        ...
        case OP.CCMP: {
          if (evalCond(tok[5],regs)) { const b=tok[2]==='R'?regs.readInt(tok[3]):BigInt(tok[3]); subWithFlags(regs,regs.readInt(tok[1]),b); }
          else { const n=Number(tok[4]); regs.N=!!(n&8); regs.Z=!!(n&4); regs.C=!!(n&2); regs.V=!!(n&1); }
          this.pc++; break;
        }
        case OP.CCMN: {
          if (evalCond(tok[5],regs)) { const b=tok[2]==='R'?regs.readInt(tok[3]):BigInt(tok[3]); addWithFlags(regs,regs.readInt(tok[1]),b); }
          else { const n=Number(tok[4]); regs.N=!!(n&8); regs.Z=!!(n&4); regs.C=!!(n&2); regs.V=!!(n&1); }
          this.pc++; break;
        }

        // Atomic / exclusive (simulated)
        ...
        case OP.LDARH: case OP.LDAXRH: case OP.LDXRH: {
          const base = regs.readInt(tok[2]);
          regs.writeInt(tok[1], regs.memReadByte(base) | (regs.memReadByte(base+1n)<<8n));
          this.pc++; break;
        }
        case OP.STLR: case OP.STXR: case OP.STLXR: {
          const sidx = op===OP.STLR?1:2, bidx = op===OP.STLR?2:3;
        ...
        case OP.STLRB: case OP.STXRB: case OP.STLXRB: {
          const sidx=op===OP.STLRB?1:2, bidx=op===OP.STLRB?2:3;
        ...
        case OP.STLRH: case OP.STXRH: case OP.STLXRH: {
          const sidx=op===OP.STLRH?1:2, bidx=op===OP.STLRH?2:3;
          const base=regs.readInt(tok[bidx]), v=regs.readInt(tok[sidx]);
          regs.memWriteByte(base,v&0xffn); regs.memWriteByte(base+1n,(v>>8n)&0xffn);
          if (op!==OP.STLRH) regs.writeInt(tok[1],0n);
          this.pc++; break;
        }
        case OP.LDXP: case OP.LDAXP: {
          const base=regs.readInt(tok[3]);
          regs.writeInt(tok[1],regs.memRead(base)); regs.writeInt(tok[2],regs.memRead(base+8n));
          this.pc++; break;
        }
        case OP.STXP: case OP.STLXP: {
          const base=regs.readInt(tok[4]);
          regs.memWrite(base,regs.readInt(tok[2])); regs.memWrite(base+8n,regs.readInt(tok[3]));
          regs.writeInt(tok[1],0n); this.pc++; break;
        }

        case OP.PRFM: case OP.PRFUM:
          this.pc++; break;
        ...
        case OP.POP: {
          ...
          if (item !== undefined) { regs.writeInt(tok[1], item.val); if (item.str !== null) regs.writeStr(tok[1], item.str); }
          this.pc++; break;
        }

        case OP.SWP: {
          const a=regs.readInt(tok[1]),b=regs.readInt(tok[2]),sa=regs.readStr(tok[1]),sb=regs.readStr(tok[2]);
          regs.writeInt(tok[1],b); regs.writeInt(tok[2],a); regs.writeStr(tok[1],sb); regs.writeStr(tok[2],sa);
          this.pc++; break;
        }
        ...
        case OP.HALT: this.running = false; break;
        ...
        case OP.GETI: {
          const line = await this.readLine();
          regs.writeInt(tok[1], BigInt(parseInt((line||'').trim(), 10) || 0));
          this.pc++; break;
        }

        case OP.GETS: {
          const line = await this.readLine();
          regs.writeStr(tok[1], line); regs.writeInt(tok[1], BigInt(line.length));
          this.pc++; break;
        }

        case OP.RAND: regs.writeInt(tok[1], BigInt(Math.floor(Math.random() * Number.MAX_SAFE_INTEGER))); this.pc++; break;
        case OP.TIME: regs.writeInt(tok[1], BigInt(Date.now())); this.pc++; break;

        case OP.CLRR: regs.writeInt(tok[1], 0n); regs.writeStr(tok[1], null); this.pc++; break;
        ...
        case OP.ABS: { const v=BigInt.asIntN(64,regs.readInt(tok[1])); regs.writeInt(tok[1],v<0n?-v:v); this.pc++; break; }
        case OP.MAX2: { const a=regs.readInt(tok[2]),b=regs.readInt(tok[3]); regs.writeInt(tok[1],a>b?a:b); this.pc++; break; }
        case OP.MIN2: { const a=regs.readInt(tok[2]),b=regs.readInt(tok[3]); regs.writeInt(tok[1],a<b?a:b); this.pc++; break; }
        case OP.MOD: { const a=regs.readInt(tok[2]),b=regs.readInt(tok[3]); regs.writeInt(tok[1],b===0n?0n:a%b); this.pc++; break; }
        ...
        case OP.MEMCPY: {
          const len=Number(regs.readInt(tok[3])), s=regs.readStr(tok[2]);
          if (s!==null) { regs.writeStr(tok[1],s.slice(0,len)); }
          else { const bs=regs.readInt(tok[2]),bd=regs.readInt(tok[1]); for(let i=0n;i<BigInt(len);i++) regs.memWriteByte(bd+i,regs.memReadByte(bs+i)); }
          this.pc++; break;
        }
        case OP.MEMSET: {
          const len=Number(regs.readInt(tok[3])),val=regs.readInt(tok[2])&0xffn,base=regs.readInt(tok[1]);
          for(let i=0n;i<BigInt(len);i++) regs.memWriteByte(base+i,val);
          this.pc++; break;
        }
        case OP.STRCPY: { const s=regs.readStr(tok[2])??''; regs.writeStr(tok[1],s); regs.writeInt(tok[1],BigInt(s.length)); this.pc++; break; }
        case OP.STRCAT_OP: { const cat=(regs.readStr(tok[1])??'')+(regs.readStr(tok[2])??''); regs.writeStr(tok[1],cat); regs.writeInt(tok[1],BigInt(cat.length)); this.pc++; break; }
        case OP.STRCMP_OP: {
          const a=regs.readStr(tok[2])??'', b=regs.readStr(tok[3])??'';
          const r=a<b?-1n:a>b?1n:0n; regs.writeInt(tok[1],r); regs.Z=r===0n; regs.N=r<0n;
          this.pc++; break;
        }
        case OP.STRLEN: { const s=regs.readStr(tok[2]); regs.writeInt(tok[1],s!==null?BigInt(s.length):0n); this.pc++; break; }
        case OP.ITOA: { const v=regs.readInt(tok[2]).toString(); regs.writeStr(tok[1],v); regs.writeInt(tok[1],BigInt(v.length)); this.pc++; break; }
        case OP.ATOI: {
          const s=regs.readStr(tok[2])??'0';
          try { regs.writeInt(tok[1],BigInt(parseInt(s.trim(),10))); } catch { regs.writeInt(tok[1],0n); }
          this.pc++; break;
        }

        case OP.CALL: regs.lr=BigInt(this.pc+1); this.callStack.push(this.pc+1); this.pc=Number(tok[1]); break;
        ...
        case OP.LDR_LIT: regs.writeInt(tok[1], BigInt(tok[2]||0)); this.pc++; break;
  ...
  const api = {
    ...
    /** Called with text written to stderr / debug */
    onError: (text) => { console.error(text); },

    /**
     * Called when the program needs a line of input (geti / gets / svc read).
     * Must return a Promise<string>.
     * Default implementation uses browser prompt(); replace with your own UI.
     */
    ...
    /**
     * Load bytecode from a URL (fetch).
     * @param {string} url  — path/URL to the .wassm file
     * @param {Function} [callback]  — called when ready (no args)
    ...
    /**
     * Load bytecode from a raw string (no fetch needed).
     ...
     */
    execute() {
      if (!this._code) return Promise.reject(new Error('No bytecode loaded. Call init() first.'));
